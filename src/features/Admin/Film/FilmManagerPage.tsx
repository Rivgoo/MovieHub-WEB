...
const handleEditFilm = async (film: ContentDto) => {
  try {
    ...
    console.error('Помилка при отриманні повної інформації про акторів:', error);
  }
};
    ...
    if (error?.response?.data?.code === "ContentActor.AlreadyExists") {
      ...
      throw error;
       ...
       if (data.poster && typeof data.poster === 'string' && data.poster.startsWith('data:')) {
        ...
        await contentApi.uploadPoster(contentId, { base64Image: cleanedBase64Poster });
      }

      if (data.banner && typeof data.banner === 'string' && data.banner.startsWith('data:')) {
        const cleanedBase64Banner = data.banner.replace(/^data:image\/\w+;base64,/, '');
        await contentApi.uploadBanner(contentId, { base64Image: cleanedBase64Banner });
      }
      ...
      initialData={
        editingFilm
          ? {
              ...
              actors: editingFilm.actors?.map(actor => ({
              id: actor.id,
              RoleName: actor.RoleName || '',
              fullName: actor.fullName || `${actor.firstName ?? ''} ${actor.lastName ?? ''}`.trim(),
